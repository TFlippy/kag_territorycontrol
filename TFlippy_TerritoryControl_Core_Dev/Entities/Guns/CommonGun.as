...
//Raycast Gun Init
void GunInitRaycast(CBlob@ this, bool isAutomatic, f32 fireDamage, f32 fireRange, u32 fireDelay, u32 clipSize, f32 ammoUsageFactor, u32 reloadTime,
                    bool shotgunReload, u32 shotgunEndDelay, u8 bulletCount, f32 bulletJitter, string ammoItem, bool soundFireLoop,
                    SoundInfo soundFire, SoundInfo soundReload, SoundInfo soundDelayed, u32 soundDelayedDelay, Vec2f lineOffset)
{
	AttachmentPoint@ ap = this.getAttachments().getAttachmentPointByName("PICKUP");
	if (ap !is null) ap.SetKeysToTake(key_action1);
...
//Projectile gun init
void GunInitProjectile(CBlob@ this, bool isAutomatic, string fireProj, f32 fireProjSpeed, u32 fireDelay, u32 clipSize, f32 ammoUsageFactor, 
                       u32 reloadTime, bool shotgunReload, u32 shotgunEndDelay, string ammoItem, bool soundFireLoop, SoundInfo soundFire, 
                       SoundInfo soundReload, SoundInfo soundDelayed, u32 soundDelayedDelay, Vec2f projOffset)
{
	...
	if (this.hasCommandID("cmd_gunReload")) return;
	this.addCommandID("cmd_gunReload");
	...
	this.set_f32    ("gun_bulletJitter",        0.0f);
	this.set_string ("gun_ammoItem",            ammoItem);
...
void Shoot(CBlob@ this)
{
	...
	if (fireProj == "")
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			//got rid of if (isServer()) so that the length variable is set on client for drawing tracer
			if (blobHit)
			{
				f32 falloff = 1;
					...
					if (hitInfos[i].blob !is null)
					{
						...
						if ((blob.isCollidable() || blob.hasTag("flesh") || force_nonsolid) && (!blob.hasTag("invincible") && blob.getTeamNum() != holder.getTeamNum()) && blob.getHealth() > 0.0f)
						{
							if (!blob.hasTag("isWeapon"))
							{
								...
								dir.Normalize();
								Vec2f blobHitPos = hitInfos[i].hitpos; //use blob hitPos, not map hitPos when server_hit

								holder.server_Hit(blob, blobHitPos, dir, dmg * 0.99f, this.get_u8("gun_hitter"), false);
								this.server_Hit(blob, blobHitPos, dir, dmg * 0.01f, this.get_u8("gun_hitter"), false); // Hack
								falloff *= 0.8f;

								// blobs that can stop bullets
								string blobName = blob.getName();
								if (((blob.hasTag("vehicle") || blob.hasTag("chicken")) && blob.hasTag("metal")) ||
								    blobName == "bison" || blobName == "cowo" || blobName == "hoob" ||
								    blobName == "iron_halfblock" || blobName == "stone_halfblock" ||
								    blobName == "royalguard" || blobName == "juggernaut" || 
									blobName == "longboat" || blobName == "warboat")
								{
									hitPos = hitInfos[i].hitpos;
									length = (blobHitPos - startPos).Length();
									break;
								}
							}
						}
			...
			//perform client logic after information is calculated
			if (isClient())
			{
				DrawLine(this.getSprite(), i, startPos, length / 32.5, jitter, this.isFacingLeft());
