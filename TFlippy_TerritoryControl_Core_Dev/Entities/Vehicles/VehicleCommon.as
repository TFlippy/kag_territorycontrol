                  ...
                  )
{
	...
	this.addCommandID("fire raycast");
...
void Vehicle_SetupAirship(CBlob@ this, VehicleInfo@ v,
                          f32 flySpeed, f32 flyAmount = 0.25f)
{
		...
		else
		{
			...
			if (loadedAmmo != 0) // shoot if ammo loaded
			{
				...
				for (u8 i = 0; i < loadedAmmo; i += v.fire_cost_per_amount)
				{
					...
					else server_FireRaycast(this, charge);
				}
...
void server_FireRaycast(CBlob@ this, const u8 charge)
{
	CBitStream params;
	params.write_u8(charge);
	this.SendCommand(this.getCommandID("fire raycast"), params);
}

float Lerp(float a,float b,float time)
{
	return a+(b-a)*Maths::Clamp01(time);
}
			...
			if (blob !is null && ap.socket)
			{
				...
				if (ap.name == "DRIVER" && !this.hasTag("immobile"))
				{
					...
					if (angle < 80 || angle > 290)
					{
						...
						{
							moveForce *= Maths::Min(Maths::Sqrt(this.get_f32("gyromat_acceleration")), 1.5f);
						}
				...
				if (blob.isMyPlayer() && (ap.name == "GUNNER" || (this.hasTag("driver is gunner") && ap.name == "DRIVER")))
				{
					// set facing
					blob.SetFacingLeft(this.isFacingLeft());
