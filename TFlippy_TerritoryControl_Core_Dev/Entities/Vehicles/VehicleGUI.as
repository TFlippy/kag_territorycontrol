...
void drawCooldownBar(CBlob@ blob, VehicleInfo@ v)
{
	if (v.cooldown_time > 0)
	{
		...
		f32 modified_last_charge_percent = float(v.max_charge_time) / float(v.last_charge);
		f32 modified_cooldown_time_percent = v.cooldown_time / (90.0 * modified_last_charge_percent);
