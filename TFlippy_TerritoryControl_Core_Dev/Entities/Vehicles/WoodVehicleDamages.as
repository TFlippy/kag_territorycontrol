...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
	...
	switch (customData)
	{
		...
		case Hitters::arrow:
			dmg = this.getMass() > 1000.0f ? 1.5f : 1.0f;
			break;
	...
	if (dmg > 0 && hitterBlob !is null && hitterBlob !is this)
	{
		...
		{
			if (damageowner.getTeamNum()!=this.getTeamNum() || (this.getTeamNum()<0 || this.getTeamNum()>=7))
			{
...
void onDie(CBlob@ this)
{
	...
	if (p !is null)
	{
		...
		{
			SendGameplayEvent(createVehicleDamageEvent(this.getPlayerOfRecentDamage(),0.0f));
		}
	}
}
