...
void onTick(CBlob@ this)
{
	...
	if (this.isAttached())
	{
		...
		if (inwater)
		{
			sprite.SetEmitSoundSpeed(0.6f + (getGameTime() % 13) * 0.01f);
		}
		else
		{
				...
				if (map.getHitInfosFromArc((this.getPosition() - attackVel), -attackVel.Angle(), 30, distance, this, false, @hitInfos))
				{
					...
					for (uint i = 0; i < hitInfos.length; i++)
					{
						...
						if (hi.blob !is null) // blob
						{
							...
							{
								holder.server_Hit(hi.blob, hi.hitpos, attackVel, 1.6f, Hitters::drill);   //extra burn damage
							}
							hitsomething = true;
							hitblob = true;
						}
						else // map
						{
							...
							if (tile >= 396 && tile <= 412)
							{
								this.getSprite().PlaySound("/metal_stone.ogg");
								this.server_Hit(this, this.getPosition(), attackVel, 0.15f, Hitters::drill); // Plasteel almost beats mithril drill
								sparks(hi.hitpos, 1, 1);
								this.server_HitMap(hi.hitpos, attackVel, 0.25f, Hitters::drill);
							}
							else
							{
							...
							ShakeScreen(5.0f, 32, this.getPosition());
