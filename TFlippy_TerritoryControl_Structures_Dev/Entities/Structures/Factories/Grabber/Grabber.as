	...
	if (!this.getInventory().isFull() && resource_name != "")
	{
		...
		if (getMap().getBlobsInRadius(this.getPosition(), 64, @blobs))
		{
			for (uint i = 0; i < blobs.length; i++)
			{
				...
				if (b.getConfig() == "grabber") continue;

				u8 team = b.getTeamNum();

				if(b.getInventory() !is null && (team == my_team || team >= 100))
				{
					if (!b.isInventoryAccessible(this) || !b.hasTag("extractable")) continue;
...
void GetButtonsFor( CBlob@ this, CBlob@ caller )
{
    ...
    if(carried.getConfig() != "gyromat"){
        CBitStream params;
        params.write_u16(caller.getNetworkID());
        params.write_u16(carried_netid);

        CButton@ button_withdraw = caller.CreateGenericButton(11, Vec2f(0, 0), this, this.getCommandID("fetcher_set"), "Set Resource", params);
...
{
	if(forBlob.getShape().isStatic())return true;
