		...
		if (bullets !is null && bullets.length > 0)
		{
			...
			for (u32 i = 0; i < bullets.length; i++)
			{
				...
				if (bullet !is null)
				{
					...
					Vec2f tl = Vec2f(x - size_x, y - (size_y * 0.50f));
					Vec2f tr = Vec2f(x - size_x, y + (size_y * 0.50f));
					...
					SColor color = bullet.color;
					SColor color_back = bullet.color;
					color_back.setAlpha(color.getAlpha() * 0.50f);
					
					
					// color.setAlpha(color.getAlpha() * (1.00f -  Maths::Pow(bullet.distance / bullet.distance_target, 8)));
					// color.setAlpha(255);
					vertices.push_back(Vertex(tl, 0, Vec2f(0, 0), color_back));
					vertices.push_back(Vertex(tr, 0, Vec2f(1, 0), color_back));
					...
					bullet.distance = Maths::Clamp((bullet.currentPos - bullet.startPos).getLength() + (speed + XORRandom(10)), 0, bullet.distance_target);
