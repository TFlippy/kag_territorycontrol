...
void onInit(CBlob@ this)
{
	...
	CSpriteLayer@ tracer = sprite.addSpriteLayer("tracer", "GatlingGun_Tracer.png", 32, 1, this.getTeamNum(), 0);
	if (tracer !is null)
	{
		Animation@ anim = tracer.addAnimation("default", 0, false);
		anim.AddFrame(0);
		
		tracer.SetOffset(miniGun_offset);
		tracer.SetRelativeZ(-100.0f);
		tracer.SetVisible(false);
		tracer.setRenderStyle(RenderStyle::additive);
	}

	CSpriteLayer@ mini = sprite.addSpriteLayer("minigun", "Helichopper_Gun.png", 16, 16, this.getTeamNum(), 0);
    if (mini !is null)
    {
	...
	CSpriteLayer@ rocket = sprite.addSpriteLayer("rocketlauncher", "Helichopper_Launcher.png", 16, 16, this.getTeamNum(), 0);
	if (rocket !is null)
	{
...
void ShootGun(CBlob@ this, f32 angle,Vec2f gunPos)
{
	...
	Vec2f dir = Vec2f(sign, 0.0f).RotateBy(angle + this.getAngleDegrees());
	...
	Vec2f endPos = startPos + dir * 1000;
	Vec2f hitPos;
...
void DrawLine(CSprite@ this, Vec2f offset, f32 length, f32 angle, bool flip)
{
	...
	tracer.SetOffset(offset + (flip?Vec2f(5,-0.5):Vec2f(5,-0.5)));
